...
crossIcon = document.querySelector('.fas.fa-times'),
...
function data(information, word){
    ...
    for(var i = 0; i<5; i++){
        if(information[0].meanings[0].definitions[0].synonyms[i] !== undefined){
            synonyms.push(information[0].meanings[0].definitions[0].synonyms[i]);
        }else{
            synonyms.push('');
        }
...
function fetchData(word){
    ...
    fetch(url).then(res=>res.json()).then(info => data(info, word));
}
